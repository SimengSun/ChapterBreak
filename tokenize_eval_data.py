...
def parse_args():
	...
	parser.add_argument("--suffix-size", type=int, default=128)
...
def tokenize(args, data, chunk_size=128, single_chunk=False):
	'''
		tokenize the data with roberta tokenizer and chunk input to chunk_size
		each chunk starts from a new sentence unless content from previous chunk
		overflows the current chunk
	'''
...
def tokenize_encode(split_name, args, data):
	all_data = {}
	for book_id in tqdm(data):
		...
		for example in data[book_id]:
			...
			all_data[book_id].append(
				{
					'ctx_vec': encode(ctx_ids) if not args.tokenize_only else ctx_ids,
					'pos_vec': encode(pos_ids) if not args.tokenize_only else pos_ids,
					'negs_vec': [encode(neg_ids) for neg_ids in negs_ids] if not args.tokenize_only else negs_ids
				}
			)

	ctx_size = os.path.basename(args.input_path).strip('.json').split('_')[-1]
	print('ctx_size')
	with open(os.path.join(args.output_path, f'{split_name}_ctx{ctx_size}.pkl'), "wb") as f:
		pickle.dump(all_data, f, protocol=pickle.HIGHEST_PROTOCOL)

def encode_file(args, tokenize_only=False):

	try:
		with open(os.path.join(args.input_path), "r") as f:
			data = f.read()
