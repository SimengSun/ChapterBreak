...
def prepare_data(args):
    ...
    def construct_dl(Dataset, chunk_size=128, fileids=None):
        kwargs = {"chunk_size": chunk_size}
        kwargs['max_books'] = 500 if args.debug else args.max_books
        if fileids is not None:
            kwargs['fileids'] = fileids
        # build dataset and dataloader
        ds = Dataset(args,
                     args.data_path, 
                     args.max_tokens_per_batch // chunk_size,
                     args.split,
                     **kwargs)
        vds = Dataset(args, args.data_path, 
                     args.max_tokens_per_batch // chunk_size, 
                     "valid-tok",
                    **kwargs)
        print(f"num_chunks {args.max_tokens_per_batch // chunk_size} chunk_size {chunk_size}")

        # using SequentialSampler to get most in-book negatives
        sampler = SequentialSampler(ds)
        
        dl = DataLoader(ds, sampler=sampler, batch_size=args.batch_size)
        vdl = DataLoader(vds, sampler=SequentialSampler(vds), batch_size=args.batch_size)
        return dl, vdl

    CHUNK_SIZES = args.chunk_size_list 

    file_lst = [fn for fn in os.listdir(os.path.join(args.data_path, args.split)) if 'data' not in fn]
    file_lst += [fn for fn in os.listdir(os.path.join(args.data_path, args.split.replace('train', 'valid'))) if 'data' not in fn]
    if args.debug:
        file_lst = file_lst[:10]
    random.shuffle(file_lst)
    shard_size = len(file_lst) // len(CHUNK_SIZES)
    for i, chunk_size in enumerate(CHUNK_SIZES):
        files = file_lst[i*shard_size:(i+1)*shard_size]
        if len(files) == 0:
            continue
        print(f"constructing dataloader for chunk_size {chunk_size}")
        dl, vdl = construct_dl(TextDataset, chunk_size=chunk_size, 
                            fileids=files)
        dl_lst.append(dl)
        vdl_lst.append(vdl)

    return dl_lst, vdl_lst
